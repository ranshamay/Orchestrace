...
import {
  ...
  selectSessionViewState,
} from "./app/selectors/sessionViewSelectors";
...
export default function App() {
  ...
  const [authUser, setAuthUser] = useState<
    AppAuthStatusResponse["user"] | null
  >(null);
  ...
  const [copyTraceState, setCopyTraceState] = useState<{
    sessionId: string;
    state: "idle" | "copied" | "failed";
  }>({ sessionId: "", state: "idle" });
  const [settingsSaveToastState, setSettingsSaveToastState] =
    useState<SettingsSaveToastState>("idle");
  const [settingsSaveToastMessage, setSettingsSaveToastMessage] = useState("");
  const [nodeTokenStreams, setNodeTokenStreams] = useState<
    Record<string, NodeTokenStream>
  >({});
  const [observerState, setObserverState] =
    useState<SessionObserverState | null>(null);
  ...
  const { setSessionSelection } = useSessionSelectionController({
    selectedSessionId,
    sessions,
    setSelectedSessionId,
  });
  ...
  } = useLlmControlsModalState();

  const onSettingsSaveStatus = useCallback(
    (state: Exclude<SettingsSaveToastState, "idle">, message: string) => {
  ...
  useSessionsStatusStream({
    enabled: canUseRealtimeApis,
    selectedSessionId,
    setSelectedSessionId: setSessionSelection,
    setSessions,
  });
  useSessionPolling({
    enabled: canUseRealtimeApis,
  ...
  useEffect(() => {
    ...
    const handleProviderReauthRequired = (event: Event) => {
      ...
      onSettingsSaveStatus(
        "error",
        `${providerLabel} needs re-login. Reconnect it in Settings > Providers.`,
      );
    ...
    window.addEventListener(
      PROVIDER_REAUTH_REQUIRED_EVENT,
      handleProviderReauthRequired as EventListener,
    );
    return () => {
      window.removeEventListener(
        PROVIDER_REAUTH_REQUIRED_EVENT,
  ...
  const selectedSession = useMemo(
    () => selectCurrentSession(sessions, selectedSessionId),
  ...
  const {
    ...
    composerMode,
  } = useMemo(() => selectSessionViewState(selectedSession), [selectedSession]);
  ...
  const persistPhaseDefaults = useCallback(
    (next: {
  ...
  const setDefaultPlanningProvider = useCallback(
    (nextProvider: string) => {
      const normalizedProvider =
        typeof nextProvider === "string" ? nextProvider.trim() : "";
      const resetModel =
        normalizedProvider !== defaultLlmControls.planningProvider;
  ...
  const setDefaultPlanningModel = useCallback(
    (nextModel: string) => {
      const normalizedModel =
        typeof nextModel === "string" ? nextModel.trim() : "";
      setDefaultLlmControls((current) => ({
        ...current,
  ...
  const setDefaultImplementationProvider = useCallback(
    (nextProvider: string) => {
      const normalizedProvider =
        typeof nextProvider === "string" ? nextProvider.trim() : "";
      const resetModel =
        normalizedProvider !== defaultLlmControls.implementationProvider;
      const nextModel = resetModel
        ? ""
        : defaultLlmControls.implementationModel;
  ...
  const setDefaultImplementationModel = useCallback(
    (nextModel: string) => {
      const normalizedModel =
        typeof nextModel === "string" ? nextModel.trim() : "";
      setDefaultLlmControls((current) => ({
  ...
  const setDefaultAgentRoleProvider = useCallback(
    (role: "router" | "reviewer" | "investigator", nextProvider: string) => {
      const normalizedProvider =
        typeof nextProvider === "string" ? nextProvider.trim() : "";
      const currentRoleConfig = defaultLlmControls.agentModels?.[role] ?? {};
      const resetModel =
        normalizedProvider !== (currentRoleConfig.provider ?? "");
  ...
  const setDefaultAgentRoleModel = useCallback(
    (role: "router" | "reviewer" | "investigator", nextModel: string) => {
      const normalizedModel =
        typeof nextModel === "string" ? nextModel.trim() : "";
  ...
  const setDefaultRouterProvider = useCallback(
    (nextProvider: string) => {
  ...
  const setDefaultRouterModel = useCallback(
    (nextModel: string) => {
  ...
  const setDefaultReviewerProvider = useCallback(
    (nextProvider: string) => {
  ...
  const setDefaultReviewerModel = useCallback(
    (nextModel: string) => {
  ...
  const setDefaultInvestigatorProvider = useCallback(
    (nextProvider: string) => {
  ...
  const setDefaultInvestigatorModel = useCallback(
    (nextModel: string) => {
  ...
  const handleStartNewSessionDraft = useCallback(() => {
    ...
    const nextPlanningProvider =
      defaultLlmControls.planningProvider || workPlanningProvider;
    const nextPlanningModel =
      defaultLlmControls.planningModel || workPlanningModel || workModel;
    const nextImplementationProvider =
      defaultLlmControls.implementationProvider ||
      workProvider ||
      nextPlanningProvider;
    const nextImplementationModel =
      defaultLlmControls.implementationModel ||
      workModel ||
      workPlanningModel ||
      nextPlanningModel;
  ...
  useHotkeys([
    {
      ...
      ctrlKey: false,
      onTrigger: () => {
        setIsQuickPromptOpen((current) => !current);
      },
    },
    {
      key: "k",
  ...
  const modalWorkProvider = modalTargetsPlanningPhase
    ? workPlanningProvider
    : workProvider;
  const modalWorkModel = modalTargetsPlanningPhase
    ? workPlanningModel
    : workModel;
  const modalSetWorkModel = modalTargetsPlanningPhase
    ? setWorkPlanningModel
    : setWorkModel;
  ...
  const timelineItems = useMemo(
    () => buildTimelineItems(selectedSession, chatMessages),
  ...
  const liveReasoning = useMemo(() => {
    ...
    if (
      selectedLlmStatus.state === "using-tools" ||
    ...
    for (const [taskId, stream] of Object.entries(nodeTokenStreams)) {
      ...
      if (
        !latest ||
        Date.parse(stream.updatedAt) > Date.parse(latest.updatedAt)
      ) {
        latest = {
          taskId,
          phase: stream.phase,
  ...
  const setDefaultPlanningNoToolGuardMode = useCallback(
    (next: "enforce" | "warn") => {
  ...
  const handleSetQuickStartMode = useCallback(
    (next: boolean) => {
  ...
  const handleSetQuickStartMaxPreDelegationToolCalls = useCallback(
    (next: number) => {
      setQuickStartMaxPreDelegationToolCalls(next);
      setDefaultLlmControls((current) => ({
        ...current,
        quickStartMaxPreDelegationToolCalls: next,
      }));
    },
    [setDefaultLlmControls, setQuickStartMaxPreDelegationToolCalls],
  );

  const handleSetAdaptiveConcurrency = useCallback(
    (next: boolean) => {
  ...
  const handleSetBatchConcurrency = useCallback(
    (next: number) => {
      setBatchConcurrency(next);
      setDefaultLlmControls((current) => ({
        ...current,
        batchConcurrency: next,
      }));
    },
    [setDefaultLlmControls, setBatchConcurrency],
  );

  const handleSetBatchMinConcurrency = useCallback(
    (next: number) => {
      setBatchMinConcurrency(next);
      setDefaultLlmControls((current) => ({
        ...current,
        batchMinConcurrency: next,
      }));
    },
    [setDefaultLlmControls, setBatchMinConcurrency],
  );

  const handleSetEnableTrivialTaskGate = useCallback(
    (next: boolean) => {
      setEnableTrivialTaskGate(next);
      setDefaultLlmControls((current) => ({
        ...current,
        enableTrivialTaskGate: next,
      }));
    },
    [setDefaultLlmControls, setEnableTrivialTaskGate],
  );

  const handleSetTrivialTaskMaxPromptLength = useCallback(
    (next: number) => {
      setTrivialTaskMaxPromptLength(next);
      setDefaultLlmControls((current) => ({
        ...current,
        trivialTaskMaxPromptLength: next,
      }));
    },
  ...
  const sessionSidebarProps = useMemo(
    () => ({
      ...
      onDeleteSession: async (sessionId: string) =>
        actions.handleDelete(sessionId),
      onRetrySession: async (sessionId: string) =>
        actions.handleRetrySession(sessionId),
    }),
    [
      activeTab,
      actions,
  ...
  const mainContentProps = {
    ...
    chatFirstTokenLatencyMs: chatStream.firstTokenLatencyMs,
    chatWaitingForFirstToken: chatStream.waitingForFirstToken,
    chatActiveToolCalls: chatStream.activeToolCalls,
    composerMode,
    workspaces,
    workWorkspaceId,
    workPlanningProvider,
    ...
    quickStartMaxPreDelegationToolCalls,
    onSetQuickStartMaxPreDelegationToolCalls:
      handleSetQuickStartMaxPreDelegationToolCalls,
    adaptiveConcurrency,
    ...
    observerState,
    copyTraceState:
      copyTraceState.sessionId === selectedSessionId
        ? copyTraceState.state
        : "idle",
    onCopyTrace: () => {
  ...
  const llmModalProps = buildLlmModalProps({
    ...
    onChangeWorkspace: (workspaceId) =>
      updateActiveLlmControls({ workspaceId }),
    onChangeProvider: (provider) =>
      updateActiveLlmControls(
        modalTargetsPlanningPhase
    ...
    onChangeDeliveryStrategy: (next) =>
      updateActiveLlmControls({ deliveryStrategy: next }),
    onChangeAutoApprove: (next) =>
      updateActiveLlmControls({ autoApprove: next }),
    onChangeAdaptiveConcurrency: (next) =>
      updateActiveLlmControls({ adaptiveConcurrency: next }),
    onChangeBatchConcurrency: (next) =>
      updateActiveLlmControls({ batchConcurrency: next }),
    onChangeBatchMinConcurrency: (next) =>
      updateActiveLlmControls({ batchMinConcurrency: next }),
  });
  ...
  const handleAuthenticated = useCallback(
    (result: { token: string; user?: AppAuthStatusResponse["user"] }) => {
