const DEFAULT_ALLOWED_SHELL_PROGRAMS = [
  'pnpm',
  'npm',
  'yarn',
  'node',
...
const DEFAULT_ALLOWED_SHELL_PROGRAMS_SET = new Set<string>(DEFAULT_ALLOWED_SHELL_PROGRAMS);
const DEFAULT_SHELL_COMMAND_START = new RegExp(`^(${DEFAULT_ALLOWED_SHELL_PROGRAMS.join('|')})(?:\\s|$)`, 'i');
...
const DEFAULT_MAX_PROMPT_COMMAND_LENGTH = 200;
...
export function parseShellCommandToArgv(command: string): ShellExecutionValidation {
  ...
  if (!DEFAULT_ALLOWED_SHELL_PROGRAMS_SET.has(normalizedProgram)) {
    return {
      ok: false,
...
export function extractShellCommand(prompt: string): string | undefined {
  ...
  if (
    FORBIDDEN_SHELL_LINE_BREAKS.test(normalized)
    || FORBIDDEN_SHELL_CONTROL_CHARS.test(normalized)
    || normalized.length > DEFAULT_MAX_PROMPT_COMMAND_LENGTH
  ) {
    return undefined;
  }
  ...
  if (!DEFAULT_SHELL_COMMAND_START.test(command)) return undefined;
...
export function validateExecutableShellCommand(input: string): ShellExecutionValidation {
  const normalized = input.trim();
  if (!normalized) {
    return {
      ok: false,
      reason: 'Blocked non-command payload: command was empty.',
    };
  }
  ...
  return parseShellCommandToArgv(normalized);
}