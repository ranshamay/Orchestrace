  ...
  const phaseRules =
    role === 'planner'
      ...
      : [
          ...
          "For search_files calls, keep the search pattern only in query, set repository/file-root scope in path (use '.' for repo-wide search), and use glob for file filtering; never embed path/glob fragments in query or mixed path-glob strings in path.",
