    ...
    role === 'planner'
      ...
      : [
          ...
          'Use subagent_spawn or subagent_spawn_batch for parallelizable slices and delegate only relevant context to each sub-agent.',
                    'Prefer targeted read_file/read_files on known paths from plan context before any broad discovery.',
          'If grep-like discovery is still needed, use scoped run_command/run_command_batch with grep/find against likely directories instead of search_files.',
          'Avoid repeated broad reconnaissance once required files are known; proceed directly to edits.',
