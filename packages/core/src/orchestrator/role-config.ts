    ...
    role === 'planner'
      ...
      : [
          ...
          'Use subagent_spawn or subagent_spawn_batch for parallelizable slices and delegate only relevant context to each sub-agent.',
                    'For ripgrep-backed search_files calls, use a first-attempt safe query strategy: single-token patterns (e.g., log.watcher), explicit escaped-space regex (e.g., log\\swatcher), or parallel single-word searches instead of multi-word space queries.',
