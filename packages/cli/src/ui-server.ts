    ...
    phase === 'chat'
      ? [
          ...
          'When asked to inspect or change todos/agent graph, call the corresponding tools instead of simulating success.',
                    'After search identifies independent files, issue one read_files batch covering all of them (with concurrency) instead of repeated one-by-one read_file calls.',
        ...
        ? [
          ...
          'If session guard thresholds are exceeded, force implementation and continue execution.',
            ...
            'For independent nodes, use subagent_spawn_batch so work runs in parallel.',
                        'After search identifies independent files, issue one read_files batch covering all of them (with concurrency); avoid repeated single-file reads across turns when possible.',
