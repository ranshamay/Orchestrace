...
export function parseRealtimeFindingsForTest(
  text: string,
  allowedCategories: FindingCategory[],
  triggerPhase: string,
): RealtimeFinding[] {
  return parseRealtimeFindings(text, allowedCategories, triggerPhase);
}
...
function parseRealtimeFindings(
  ...
  try {
    ...
    return parsed.findings
      ...
      .filter((f) => isValidRealtimeFindingCandidate(f))
      .filter((f) => allowedCategories.includes(f.category as FindingCategory))
      .map((f): RealtimeFinding => {
