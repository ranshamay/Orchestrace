...
- Each finding must include an evidence array with concrete log-derived proof points
  ...
  "findings": [
    {
      ...
      "evidence": ["Concrete log-derived evidence point"],
    ...
    return raw
      ...
      .map((f: Record<string, unknown>) => ({
        ...
        evidence: Array.isArray(f.evidence)
          ? f.evidence.filter((entry: unknown) => typeof entry === 'string').map((entry: unknown) => String(entry))
          : [],
