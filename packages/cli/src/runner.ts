...
function buildSystemPrompt(config: SessionConfig, phase: 'planning' | 'implementation', effort: TaskEffort = 'high'): string {
  ...
  const phaseRules = phase === 'planning'
    ? [
      ...
      'Before scheduling read_file for uncertain paths, validate existence first via list_directory/search_files or read_files(required:false).',
      'If a path is confirmed missing, do not repeatedly retry direct reads for that same path without new evidence.',
      '',
