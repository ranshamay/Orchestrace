      ...
      const first = await daemon.ingestSessionObserverFindings('session-123', [
        {
          ...
          evidence: 'Observed repeated cross-session mutable-state writes without synchronization in session traces.',
      ...
      const equivalentQueued = await daemon.ingestSessionObserverFindings('session-456', [
        {
          ...
          evidence: 'Equivalent architecture finding indicates unsafely shared mutable state across sessions.',
      ...
      const first = await daemon.ingestLogWatcherFindings([
        {
          ...
          evidence: 'Logs show malformed payload responses triggering repeated retry loops.',
          relevantFiles: ['packages/cli/src/ui-server.ts'],
        },
      ]);
      ...
      const performance = await daemon.ingestLogWatcherFindings([
        {
          ...
          evidence: 'Log watcher captured high-frequency polling bursts under load.',
        },
      ]);
        ...
        findings: [
          {
            ...
            evidence: 'Repeated timeout lines appear with no capped retry behavior.',
        ...
        JSON.stringify([
          {
            ...
            evidence: 'Legacy finding evidence retained from prior observer run.',
      ...
      const first = await daemon.ingestLogWatcherFindings([
        {
          ...
          evidence: 'Current-process logs show issue A error pattern recurrence.',
        },
      ]);
      ...
      const second = await daemon.ingestLogWatcherFindings([
        {
          ...
          evidence: 'Current-process logs show issue B error pattern recurrence.',
        },
      ]);
