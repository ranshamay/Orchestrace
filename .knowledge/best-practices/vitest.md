...
## Overview
Vitest is part of the Orchestrace stack and should be used with explicit conventions so implementation remains reliable, maintainable, and easy to review.

## Why It Matters in This Repo
- Package/technology: `vitest`
...
- Tests should be deterministic, isolated, and easy to diagnose when failing.
- Assert behavior and contracts, not incidental implementation details.

## Recommended Workflow
1. Read this guide before implementing changes in files that rely on Vitest.
...
describe('fetchRun', () => {
  ...
  it('parses payload', async () => {
    vi.spyOn(global, 'fetch').mockResolvedValue(new Response(JSON.stringify({ id: 'r1' })))
    const res = await fetch('/api/run/r1').then(r => r.json())
...
```ts
// bad: async test without await/return
it('loads', () => {
  fetch('/api')
...
- Using Vitest features inconsistently across packages.
...
## Definition of Done for Vitest Changes
